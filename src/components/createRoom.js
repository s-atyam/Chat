...
const joinRoom = (props) => {
  ...
  const sendUser = (e) => {
    props.socket.emit('send-user-create', { 'name': stateVariable.name, "userId": props.socket.id, "roomCode": stateVariable.room });
  }
  ...
  return (
    <div className="flex items-center justify-center flex-col bg-slate-200 w-screen h-screen">
      <div className="pb-10 flex items-center justify-center flex-col">
        <p className="font-thin text-slate-600 text-xl">Create rooms for your custom room code</p>
      </div>
      <form className="flex items-center justify-center rounded-2xl flex-col w-1/4 min-w-min h-72  bg-slate-200" style={{ "boxShadow": "-6px -6px 8px rgba(245,245,245,0.5),6px 6px 8px rgba(70,70,70,0.12)" }}>
        <div className="w-5/6 my-2">
          ...
          <input className="bg-slate-200 border rounded-md ml-2 h-9 w-10/12 text-sm text-slate-500 px-2" placeholder='Enter your name...' style={{ "outline": "none", "border": "none", "boxShadow": " inset -3px -3px 5px rgba(255,255,255,0.5), inset 3px 3px 5px rgba(70,70,70,0.12)" }} type="text" name="myName" onChange={(event) => { stateVariable.setName(event.target.value) }} />
        </div>
        <div className="w-5/6 my-2">
          ...
          <input className="bg-slate-200 border rounded-md ml-2 h-9 w-10/12 text-sm text-slate-500 px-2" style={{ "outline": "none", "border": "none", "boxShadow": " inset -3px -3px 5px rgba(255,255,255,0.5), inset 3px 3px 5px rgba(70,70,70,0.12)" }} type="password" name="pass" onChange={(event) => { stateVariable.setRoom(event.target.value) }} placeholder='Leave blank for random room code...' />
        </div>
        <Link className='w-1/3' to={`/chat`}>
          <button className="my-6 bg-slate-200 text-slate-500 rounded-md text-lg border hover:text-slate-700 h-10 w-full" style={{ "border": "none", "boxShadow": "-4px -4px 5px rgba(255,255,255,0.5),4px 4px 5px rgba(70,70,70,0.12)" }} type="submit" onClick={sendUser}>Join Room</button>
        </Link>
      </form>
      <div>
