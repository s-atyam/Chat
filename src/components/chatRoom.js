...
const chatRoom = (props) => {
  ...
  const sendChat = (e) => {
    e.preventDefault();
    props.socket.emit('chat', { message:stateVariable.message , "iddiv": props.socket.id });
    ...
    <div className='flex items-center justify-center flex-col bg-slate-300 w-screen h-screen'>
      ...
      <div onChange={setTimeout(()=>{document.getElementById('mainArea').scrollTop = document.getElementById('mainArea').scrollHeight},0)} id='mainArea' className='bg-slate-200 rounded-md w-1/2 min-w-min h-3/5 px-2 py-2 overflow-auto'>
        {stateVariable.chat.map((payload, index) => {
          return (
            <p className={`bg-slate-300 rounded-md h-8 w-fit ${props.socket.id===payload.iddiv?'ml-auto':'mr-auto'} text-lg text-slate-600 px-4 mb-1`} key={index}>{payload.message}</p>
          )
        })}
      </div>
      <div className='flex items-center justify-center w-1/2 min-w-min mt-2'>
        <input className='bg-slate-100 rounded-md h-12 w-full text-lg text-slate-500 mr-2 pl-4' style={{ "outline": "none", "border": "none" }} type='text' name='chat' placeholder={props.socket.id} value={stateVariable.message} onChange={(e)=>{stateVariable.setMessage(e.target.value)}}/>
