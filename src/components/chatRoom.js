...
const chatRoom = (props) => {
  ...
  return (
    <div className='flex items-center justify-center flex-col bg-slate-300 w-screen h-screen'>
      ...
      <form className='w-1/2 min-w-min h-3/5'>
      <div onChange={() => { setTimeout(() => { document.getElementById('mainArea').scrollTop = document.getElementById('mainArea').scrollHeight }, 0) }} id='mainArea' className='bg-slate-200 rounded-md w-full h-full px-2 py-2 overflow-auto'>
        {stateVariable.chat.map((payload, index) => {
          return (
            <div>
            <p>{}</p>
            <p className={`bg-slate-300 rounded-md h-8 w-fit ${props.socket.id === payload.userID ? 'ml-auto' : 'mr-auto'} text-lg text-slate-600 px-4 mb-1`} key={index}>{payload.name} - {payload.message}</p>
            </div>
          )
        })}
      </div>
