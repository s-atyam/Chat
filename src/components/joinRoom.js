...
const createRoom = (props) => {
  ...
  const sendUser = (e) => {
    props.socket.emit('send-user', { 'name':stateVariable.name , "userId": props.socket.id, "roomCode":stateVariable.room });
  }
    ...
    <div className="flex items-center justify-center flex-col bg-slate-200 w-screen h-screen">
      ...
      <form className="flex items-center justify-center rounded-2xl flex-col w-1/4 min-w-min h-72  bg-slate-200" style={{ "boxShadow": "-6px -6px 8px rgba(245,245,245,0.5),6px 6px 8px rgba(70,70,70,0.12)" }}>
        <div className="w-5/6 my-2">
          ...
          <input className="bg-slate-200 border rounded-md ml-2 h-9 w-10/12 text-md text-slate-500 px-2" placeholder='Enter your name...' style={{ "outline": "none", "border": "none", "boxShadow": " inset -3px -3px 5px rgba(255,255,255,0.5), inset 3px 3px 5px rgba(70,70,70,0.12)" }} type="text" name="myName" onChange={(event)=>{stateVariable.setName(event.target.value)}}/>
        </div>
        <div className="w-5/6 my-2">
          ...
          <input className="bg-slate-200 border rounded-md ml-2 h-9 w-10/12 text-md text-slate-500 px-2" style={{ "outline": "none", "border": "none", "boxShadow": " inset -3px -3px 5px rgba(255,255,255,0.5), inset 3px 3px 5px rgba(70,70,70,0.12)" }} type="password" name="pass" onChange={(event)=>{stateVariable.setRoom(event.target.value)}} placeholder='Enter room code...'/>
        </div>
